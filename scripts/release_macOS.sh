...
# Resolve Qt5 from Homebrew. Works on both Apple Silicon (/opt/homebrew)
# and Intel (/usr/local) without hardcoding the prefix.
QTDIR=$(brew --prefix qt@5 2>/dev/null || brew --prefix qt 2>/dev/null || echo /usr/local/opt/qt)
