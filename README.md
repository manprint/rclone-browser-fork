...
3.  Install dependencies `brew install git cmake rclone qt@5`
...
7.  Run `cmake .. -DCMAKE_PREFIX_PATH:PATH=$(brew --prefix qt@5)` from build folder to create makefile

    *Note: this works on both Apple Silicon (arm64, Homebrew under `/opt/homebrew`) and Intel (x86_64, Homebrew under `/usr/local`). On Apple Silicon the build is native arm64 and requires macOS 11.0 or newer. Make sure to use an arm64 `rclone` binary (`rclone` for `osx-arm64`) at runtime.*
